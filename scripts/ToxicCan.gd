...
var particles = preload("res://scenes/Particles/ToxicCanBreakParticles.tscn");
var brokentoxcan = preload("res://scenes/Objects/BrokenToxicCan.tscn");
var ps_high: Array = [];
var ps_medium: Array = [];
var ps_small: Array = [];
...
func xxx_process_destroy():
	...
	instance.name = "brokentoxiccan_%s" % get_tree().root.get_child(1).get_child(1).get_children().size();
